...
function bounceLoginContextVertical() {
    if (!Ti.App.keyboardVisible) {
        $.loginContentWrapper.animate({ center:{x:Ti.Platform.displayCaps.platformWidth / 2, y:320}, duration:150, curve:Ti.UI.ANIMATION_CURVE_EASE_IN }, function() {
            $.loginContentWrapper.animate({ center:{x:Ti.Platform.displayCaps.platformWidth / 2, y:330}, duration:100 });
        });
        ...
        $.loginContentWrapper.animate({
            center: {
                ...
                y: ((Ti.Platform.displayCaps.platformHeight - state.currentKeyboardFrameHeight) / 2) - 10
            },
            duration: 150
        }, function() {
            $.loginContentWrapper.animate({
                center: {
                    x: Ti.Platform.displayCaps.platformWidth / 2,
                    y: (Ti.Platform.displayCaps.platformHeight - state.currentKeyboardFrameHeight) / 2
                },
                duration: 100
            });
        });
...
function disableLoginBtn() {
    ...
    $.loginBtn.animate({ opacity:0.0, duration:100 });
}
...
function validateFields() {
    ...
    if (_isValidUsername) {
        ...
        $.usernameValidInd.animate({ opacity:1.0, right:0, duration:250 });
    } else {
        state.username.valid = false;
        $.usernameValidInd.animate({ opacity:0.0, right:10, duration:250 });
        $.loginBtn.animate({ opacity:0.0, right:10, duration:250 });
    }
    ...
    if (_isValidPassword) {
        ...
        if (_isValidUsername) {
            $.loginBtn.animate({ opacity:1.0, right:0, duration:250 });
        }
    } else {
...
function unfocusUsernameField(e) {
    ...
    $.clearUsernameBtn.animate({ opacity:0.0, duration:250 });
    $.usernameBg.animate({ backgroundColor:'#969696', duration:250 });

    if ($.usernameField.value.length > 0) {
        $.usernameField.color = '#333';
        $.usernameField.animate({ left:40, duration:250 });
...
function unfocusPasswordField(e) {
    ...
    $.clearPasswordBtn.animate({ opacity:0.0, duration:250 });
    $.passwordBg.animate({ backgroundColor:'#969696', duration:250 });

    if ($.passwordField.value.length > 0) {
        $.passwordField.color = '#333';
        $.passwordField.animate({ left:40, duration:250 });
...
function unfocusDomainField(e) {
    ...
    $.clearDomainBtn.animate({ opacity:0.0, duration:250 });
    ...
    if ($.domainField.value.length > 0) {
        ...
        if ($.domainField.value.length > 0) {
            ...
            $.domainField.animate({ left:40, duration:250 });
            $.domainFieldLbl.animate({ left:40, duration:250 });
        } else {
            hideDomainField();
            $.domainFieldLbl.animate({ left:40, opacity:1.0, duration:250 });
        }
    } else {
        hideDomainField();
        $.domainFieldLbl.animate({ left:40, opacity:1.0, duration:250 });
...
function focusUsernameField(e) {
    ...
    $.usernameField.animate({ left:70, duration:250 });
...
function focusPasswordField(e) {
    ...
    $.passwordField.animate({ left:70, duration:250 });
...
function focusDomainField(e) {
    ...
    $.domainField.animate({ left:70, duration:250 });
...
function focusDomainLbl(e) {
    $.domainLbl.animate({ opacity:0.0, duration:100 });
    $.domainLblTouch.animate({ opacity:1.0, duration:100 });
}
...
function focusCancelLbl(e) {
    $.cancelLbl.animate({ opacity:0.0, duration:100 });
    $.cancelLblTouch.animate({ opacity:1.0, duration:100 });
}
...
function focusLoginBtn() {
    $.loginBtn.animate({ transform:Ti.UI.create2DMatrix({ scale:0.8 }), duration:100 });
}
...
function hideDomainField() {
    if (state.domain.visible) {
        ...
        $.loginContentWrapper.animate({ height:310, duration:250 });
        $.domainBg.animate({ opacity:0.0, duration:250 }, function() {
            $.moreContainer.animate({ opacity:1.0, duration:100 });
        });
    }
}
...
function init(config) {
    ...
    $.parentContainer.width = Ti.Platform.displayCaps.platformWidth + 20;
    ...
    $.domainContainer.addEventListener('touchend', blurDomainLbl);
