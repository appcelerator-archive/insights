...
function getPlatformIcon(url) {
    ...
    var _platforms = {
        ios: {
            finder: 'apple.png',
            url: 'nav/platform-ios.png'
        },
        android: {
            finder: 'android.png',
            url: 'nav/platform-android.png'
        }
    };
...
function focusFilterField(e) {
    ...
    if (Alloy.Globals.insights.state.loggedIn && state.apps.length > 0) {
        ...
        $.filterField.animate({ left:80, duration:250 });
...
function createAppRow(config) {
    ...
    var _appRow = {
        ...
        controllers: {
            ...
            focusRow: function() {
                ...
                _appRow.views.focusBg.animate({ opacity:1.0, duration:0 });
            },
            blurRow: function() {
    ...
    _appRow.parent = Ti.UI.createTableViewRow({ selectionStyle:Ti.UI.iPhone.TableViewCellSelectionStyle.NONE, className:'appRow', backgroundImage:'', backgroundColor:'transparent', width:Ti.UI.FILL, height:50 });
    ...
    if (state.apps[_config.index].pinned) {
        _pinnedBtn = Ti.UI.createView({ index:_config.index, bubbleParent:false, width:50, height:50, backgroundImage:'nav/pinned-app-icon@2x.png', right:0 });
        _pinnedDelBtn = Ti.UI.createView({ opacity:0.0, transform:Ti.UI.create2DMatrix({ scale:0.1 }), backgroundImage:'nav/pinned-app-delete-icon.png', width:50, height:50, right:0, touchEnabled:false });
    ...
    if (state.apps[_config.index].native) {
        Ti.API.info('This is a native app...');
    }
                ...
                $.appListContainer.animate({ left:-4, opacity:1.0, duration:150, curve:Ti.UI.ANIMATION_CURVE_EASE_IN }, function() {
                    $.appListContainer.animate({ left:0, duration:100 }, function() {
                        if (state.unpinnedAppRowIndex) {
                            $.appListContainer.scrollToIndex(state.unpinnedAppRowIndex, { position:Ti.UI.iPhone.TableViewScrollPosition.MIDDLE });
...
function focusLoginBtn() {
    $.loginLbl.animate({ opacity:0.0, duration:100 });
    $.loginLblTouch.animate({ opacity:1.0, duration:100 });
}
...
function focusDemoBtn(e) {
    $.demoBg.animate({ opacity:(state.currentAppIndex === -1) ? 1.0 : 0.0, duration:100 });
    $.demoBgTouch.animate({ opacity:(state.currentAppIndex === -1) ? 0.0 : 1.0, duration:100 });
}
...
function focusRefreshBtn() {
    $.refreshIcon.animate({ opacity:0.0, duration:100 });
    $.refreshIconTouch.animate({ opacity:1.0, duration:100 });
}
