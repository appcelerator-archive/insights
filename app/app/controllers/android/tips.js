...
function processTipData() {
    state.tipsData = JSON.parse(Ti.Filesystem.getFile(state.jsonPath).read().text);
...
function getAppendText() {
    if (state.remaining[state.currentTipToShow] + 1 === state.tipTotal[state.currentTipToShow]) {
        // frist tip in set
        return '<span class="sub-highlight">Swipe left</span> to continue or <span class="sub-highlight">tap back</span> to dismiss tips for this screen...';
        ...
        return '<span class="sub-highlight">Swipe left</span> to continue, <span class="sub-highlight">swipe right</span> to view previous tip, or <span class="sub-highlight">tap back</span> to dismiss tips for this screen...';
    }
}
            ...
            <head>
                ...
                <style>
                    @font-face { 
                        ...
                        src: "file:///android_asset/Resources/fonts/TitilliumText22L-250wt";
                    }

                    @font-face { 
                        font-family: TitilliumText22L-800wt; 
                        src: "file:///android_asset/Resources/fonts/TitilliumText22L-800wt";
                    }
                    ...
                    * {
                    ...
                    div.wrapper {
                        padding: ' + lib.scaler.sv(_padding, false) + 'px; 
                        position: absolute;
                        top: ' + lib.scaler.sv(_position[0] - _padding, false) + 'px;
                        left: ' + lib.scaler.sv(_position[1], true) + 'px;
                        width: ' + lib.scaler.sv(_width - (_padding * 2), true) + 'px;
                        z-index: 0;
                    }
                    ...
                    div.bg {
                        ...
                        border-radius: ' + lib.scaler.sv(8, false) + 'px ' + lib.scaler.sv(4, true) + 'px ' + lib.scaler.sv(8, false) + 'px ' + lib.scaler.sv(4, true) + 'px;
                        opacity: 0.8;
                        z-index: 1;
                    }
                    ...
                    p { 
                        text-align: center;
                        font-size: ' + lib.scaler.sv(16, false) + 'px;
                        line-height: ' + lib.scaler.sv(24, false) + 'px;
                        margin-bottom: ' + lib.scaler.sv(15, false) + 'px;
                        color: #f2f2f2;
                        position: relative;
                        font-family: "TitilliumText22L-250wt";
                        z-index: 2;
                    ...
                    .outro {
                        font-size: ' + lib.scaler.sv(12, false) + 'px;
                        line-height: ' + lib.scaler.sv(18, false) + 'px;
                        color: #666;
                        margin-bottom: 0px;
                    }
                    ...
                    .count {
                        font-family: "TitilliumText22L-250wt";
                        font-size: ' + lib.scaler.sv(10, false) + 'px;
                        color: #aaa;
                        position: absolute;
                        bottom: ' + lib.scaler.sv(10, false) + 'px;
                        right: ' + lib.scaler.sv(15, true) + 'px;
                        z-index: 2;
                    }
                </style>
            </head>
...
function createTip(tipData) {
    var _tip          = {},
        ...
        _lblContainer = Ti.UI.createWebView({ enableZoomControls:false, backgroundColor:'transparent', html:_baseHTML.start + tipData.html + _baseHTML.end, hideLoadIndicator:true, top:0, width:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth), height:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight) });
    ...
    _tip.controllers = {
        animate: function() {
            if (_tip && _tip.state.shouldAnimate) {
                ...
                _lblContainer.animate({ top:_lblContainer.top - lib.scaler.sv(5, false), duration:2000 }, function() {
                    if (_tip && _tip.state.shouldAnimate) {
                        _lblContainer.top = _lblContainer.top - lib.scaler.sv(5, false); // #WORKAROUND
                        _lblContainer.animate({ top:_lblContainer.top + lib.scaler.sv(10, false), duration:2000 }, function() {

                            if (_tip && _tip.state.shouldAnimate) {
                                _lblContainer.top = _lblContainer.top + lib.scaler.sv(10, false); // #WORKAROUND
        ...
        hide: function(fromGoBack, fromGoForward, onHide) {
            ...
            if (fromGoBack) {
                ...
                _tip.parentView.animate({ left:lib.scaler.sv(20, true), opacity:0.0, duration:250 }, function() {
        ...
        wiggle: function() {
            _tip.parentView.animate({ center:{ x:(Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth) / 2) - lib.scaler.sv(6, true), y:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight) / 2 }, curve:Ti.UI.ANIMATION_CURVE_EASE_IN, duration:100 }, function() {
                _tip.parentView.animate({ center:{ x:(Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth) / 2) + lib.scaler.sv(3, true), y:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight) / 2 }, curve:Ti.UI.ANIMATION_CURVE_LINEAR, duration:100 }, function() {
                    _tip.parentView.animate({ center:{ x:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth) / 2, y:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight) / 2 }, duration:50 });
                });    
            });
        },
        destroy: function() {
    ...
    _tip.parentView = Ti.UI.createView({ opacity:0.0, width:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth), height:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight), touchEnabled:false });
...
function renderTip(tip) {    
    if (!state.currentTip) {
        ...
        $.tipContainer.animate({ opacity:1.0, duration:100 }, function() {
            $.parentView.visible = true; // #BUG
            $.tipContainer.visible = true; // #BUG
        ...
        destroyCurrentlyRenderedTip(fromGoBack, fromGoForward, function() {
            ...
            $.tipContainer.animate({ opacity:(state.callbacks.onComplete) ? 1.0 : 0.0, duration:(state.callbacks.onComplete) ? 0 : 100 }, function() {
                ...
                if (!state.callbacks.onComplete) {
                    $.parentView.visible = false; // #BUG
                    $.tipContainer.visible = false; // #BUG
...
function scale() {
    $.tipContainer.width = Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth),
    $.tipContainer.height = Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight);
}
...
function init(config) {
    ...
    lib.scaler = require('client/scaler')();
    ...
    // #BUG - Can't doubletap webview, event rejected
    // $.parentView.addEventListener('doubletap', function(e) {
    //     if (e.scale < 0.8) {
    //         if (state.currentTip) {
    //             state.currentTip.parentView.animate({ transform:Ti.UI.create2DMatrix({ scale:0.8 }), duration:100 }, function() {
    //                 disable();
...
exports._init          = init,
exports._isInit        = function() { return false; }, /* #TODO isInit*/
