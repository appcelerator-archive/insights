...
function hide(e) {
    ...
    if (_id) { 
        Ti.API.info('Hide called by source id: ' + _id);
    } else {
        Ti.API.info('Hide called. No source ID found...');
    }

    if (state.showing && _id !== 'eventScrollView') {
        state.showing = false;
        ...
        $.parentView.animate({ top:Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight), duration:250 });
...
function update(type, animate) {
    if (animate) {
        ...
        $.selectedCategoryIconView.backgroundImage = '/images/detail/metric-category-current-icon-' + type + '.png';
...
function scale() {
    $.parentView.width  = Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformWidth),
    $.parentView.height = Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight),
    $.parentView.top    = Alloy.Globals.insights.utils.PXToDP(Ti.Platform.displayCaps.platformHeight);

    lib.scaler.sp($.metricsContainer, { left:40, right:40, height:140 });
    lib.scaler.sp($.selectedCategoryContainer, { bottom:64, height:50 });
    lib.scaler.sp($.selectedCategoryIconView, { height:40 }, ['width']);
    lib.scaler.sp($.selectedCategorySpacer, { width:15 });
    lib.scaler.sp($.selectedCategoryLbl, { font:{ fontSize:50} });
}
